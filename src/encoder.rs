...
pub fn write_tx_blocks<T: Pixel, W: Writer>(
  ...
  rdo_type: RDOType, need_recon_pixel: bool, hashmap: HashMapVecType,
  hash_buffer: Option<HashBufferType>,
...
fn encode_partition_bottomup<T: Pixel, W: Writer>(
  ...
  if can_split {
    ...
    if best_partition != PartitionType::PARTITION_SPLIT {
      // NOTE: We have determined our last encoding was not the optimal way to do it
      // So we re-encode
...
fn encode_partition_topdown<T: Pixel, W: Writer>(
  ...
  enc_stats: &mut EncoderStats, hashmap: HashMapVecType,
  hash_buffer: Option<HashBufferType>,
) {
  if tile_bo.0.x >= ts.mi_width || tile_bo.0.y >= ts.mi_height {
    return;
    ...
    PartitionType::PARTITION_NONE => {
      ...
      let part_decision =
        if let Some(part_mode) = rdo_output.part_modes.first() {
          ...
          rdo_decision = rdo_mode_decision(
            fi,
            ts,
            cw,
            bsize,
...
pub fn encode_frame<T: Pixel>(
  ...
  let tile_group =
    encode_tile_group(fi, fs, inter_cfg, hashmap.clone(), hash_buffer.clone());
